    ...
    function createPlaceholder() {
        ...
        $.each(elMargin, function (key, value) {
            if (key !== 'vertical' && key !== 'horizontal') {
                placeholder.css('margin-' + key, 0 + 'px');             //CUSTOM margin değiştirildi 'value' -> 0
            }   
        });
    ...
    function dragStart(e) {
        ...
        drag(e,
            ...
            function finish(offset) {
                ...
                updateIndex(pageX + offset.x, pageY + offset.y);
                updatePose();
                document.getElementById('result-h-flow').value = dataHFlow.getOrder();  //CUSTOM bu aslında burada yok
            }
        );
    }
...
if('2' == $colCount){
    $containerHWidth = document.getElementById('container-h-flow').clientWidth-64;
}else{
    $containerHWidth = document.getElementById('container-h-flow').clientWidth-32;
}
if('2' == $colCount){
    if($containerHWidth>764){
        $hWidth = $containerHWidth/2-8;
        $hHeight = 200;
    }else if($containerHWidth>576){
        $hWidth = $containerHWidth/2-8;
        $hHeight = 100;
    }else{
        $hWidth = $containerHWidth;
        $hHeight = 100;
    }
}
if('4' == $colCount){
    if($containerHWidth>1015){
        $hWidth = $containerHWidth/4-8;
        $hHeight = 256;
    }else if($containerHWidth>764){
        $hWidth = $containerHWidth/3-8;
        $hHeight = 256;
    }else if($containerHWidth>607){
        $hWidth = $containerHWidth/3-8;
        $hHeight = 200;
    }else if($containerHWidth>559){
        $hWidth = $containerHWidth/2;
        $hHeight = 256;
    }else if($containerHWidth>284){
        $hWidth = $containerHWidth/2;
        $hHeight = 150;
    }else{
        $hWidth = $containerHWidth/2;
...
document.getElementById('result-h-flow').value = dataHFlow.getOrder();